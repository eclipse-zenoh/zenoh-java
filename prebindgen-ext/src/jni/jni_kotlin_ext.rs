...
impl JniExt {
    ...
    /// target triple, used by [`render_native_lib_loader_source`] to
    ...
    pub(crate) fn write_target_enum(&self, output_dir: &Path) -> Result<PathBuf, WriteKotlinError> {
        let file = KotlinFile {
            ...
            contents: render_target_source(&self.package),
        };
        Ok(file.write(output_dir)?)
    }
    ...
    pub(crate) fn write_native_lib_loader(
        ...
        let file = KotlinFile {
            ...
            contents: render_native_lib_loader_source(&self.package, lib_name),
        };
        Ok(file.write(output_dir)?)
    }
    ...
    pub(crate) fn write_jni_orphaned(
        ...
        let contents = render_jni_orphaned_source(self, registry, kotlin_types, &promoted, &class_name);
        let file = KotlinFile {
            package: self.package.clone(),
            class_name,
            contents,
...
/// `Target.kt` source — the universal 6-value host-target enum.
/// `toString()` returns the Rust target triple, matching the
/// `<target>/<target>.zip` resource layout used by the loader.
...
/// `NativeLibLoader.kt` source — the universal JVM-side native-library
/// loader. Class-init resolves the host target, locates either a local
/// `lib<name>.{dylib,so,dll}` classpath resource (dev build) or the
/// per-target `<target>/<target>.zip` packaged in a published JAR,
/// extracts to a temp file, and calls `System.load`. Referenced from
/// the generated `JNINative.init { … }` block.
