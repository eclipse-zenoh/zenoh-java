...
impl JniExt {
    ...
    pub(crate) fn write_native_handle(&self, output_dir: &Path) -> Result<PathBuf, WriteKotlinError> {
        if self.native_handle_exception_class.is_empty() {
            panic!(
                "JniExt: native_handle_exception_class is not configured — call \
                 `.native_handle_exception_class(\"pkg.ExceptionClass\")` (the Kotlin \
                 exception thrown by NativeHandle on closed-handle access)"
            );
        }
        let file = KotlinFile {
            package: self.package.clone(),
            class_name: "NativeHandle".into(),
            contents: render_native_handle_source(&self.package, &self.native_handle_exception_class),
        };
...
fn render_native_handle_source(package: &str, exception_fqn: &str) -> String {
    ...
    let import_line = if exception_fqn.contains('.') {
        format!("import {}\n", exception_fqn)
    } else {
        String::new()
    };
