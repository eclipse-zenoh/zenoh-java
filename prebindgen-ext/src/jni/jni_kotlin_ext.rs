...
impl JniExt {
    ...
    /// Emit the `internal object <jni_native_class> { external fun … }`
    /// holder under `output_dir`. One `external fun` per `#[prebindgen]`
    /// function — names mangled via `kotlin_fun_name_mangle`, parameter
    /// types and return type rendered at the JNI **wire** level (so the
    /// Kotlin declarations match the Rust extern symbols generated under
    /// `Java_<package>_<jni_native_class>_<name>`). Counterpart to the
    /// safe wrappers in `JNIWrappers.kt` and the typed-handle classes,
    /// both of which dispatch into this object.
    pub(crate) fn write_jni_native(
        ...
        let contents = render_jni_native_source(self, registry, kotlin_types);
        let file = KotlinFile {
            package: self.package.clone(),
            class_name: self.jni_native_class.clone(),
            contents,
        };
        Ok(file.write(output_dir)?)
    }
...
/// Render the `internal object <jni_native_class> { … }` holder: one
/// `external fun` per `#[prebindgen]` function, at the JNI **wire**
/// level. Parameter and return types are derived from the same registry
/// entries `render_wrapper_fn` reads, but expressed in their wire form:
/// opaque-handle params → `Long`, `kotlin_enum` params → `Int`, `Any`
/// (impl-Into Dispatch) params → `Any`, every other param uses the
/// entry's high-level Kotlin name. Opaque returns become `Long`; every
/// other return uses [`classify_return`]'s `kt_return`.
fn render_jni_native_source(
    ...
    for ident in idents {
        let (item_fn, _loc) = &registry.functions[ident];
        if let Some(line) = render_jni_native_extern(ext, item_fn, registry, &merged_types, &mut imports) {
            body.push_str(&line);
            body.push('\n');
        }
    }
...
/// Returns `None` if any parameter's input converter isn't resolved (no
/// registry entry) — same skip-rather-than-panic policy as
/// [`render_wrapper_fn`].
fn render_jni_native_extern(
    ext: &JniExt,
    f: &syn::ItemFn,
    registry: &Registry<KotlinMeta>,
    kotlin_types: &KotlinTypeMap,
    ...
    for input in &f.sig.inputs {
        ...
        // Wire form (matches what the Rust extern actually receives):
        //   opaque-handle (Borrow/Consume) → jlong → "Long"
        //   kotlin_enum                    → jint  → "Int"  (call passes `.value`)
        //   Any (impl Into<T> Dispatch)    → JObject → "Any"
        //   everything else                → entry's high-level kotlin_name
    ...
    // Return: opaque returns become `Long`; everything else uses
